      ...
      uint16_t color)
{
  ...
  while (x<y) {
    ...
    if (cornername & 0x4) {
      TFTSHIELD_1_SetPixel(x0 + x, y0 + y, color);
      TFTSHIELD_1_Set(x0 + y, y0 + x, color);
    } 
    if (cornername & 0x2) {
      TFTSHIELD_1_SetPixel(x0 + x, y0 - y, color);
      TFTSHIELD_1_SetPixel(x0 + y, y0 - x, color);
...
{
    // smarter version
  TFTSHIELD_1DrawLine(x+r  , y    , w-2*r, color); // Top
  TFTSHIELD_1DrawLine(x+r  , y+h-1, w-2*r, color); // Bottom
  TFTSHIELD_1DrawLine(x    , y+r  , h-2*r, color); // Left
  TFTSHIELD_1DrawLine(x+w-1, y+r  , h-2*r, color); // Right
  // draw four corners
